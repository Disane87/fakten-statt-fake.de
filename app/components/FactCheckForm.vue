<template>
  <div class="max-w-6xl mx-auto mt-6 flex flex-col gap-8 lg:flex-row items-start">
    ...
    <div class="flex-1 w-full bg-panel p-8 rounded-2xl shadow-xl border border-border transition-colors">
      ...
      <textarea
        v-model="text"
        rows="5"
        class="w-full p-3 rounded-xl bg-muted text-card-foreground placeholder:text-muted-foreground
               border border-border focus:outline-none focus:ring-2 focus:ring-brand/40 transition"
        placeholder="Beispiel: »Deutschland macht sich gut in der Welt.«"
        :disabled="loading"
      />
      ...
      <button
        ...
        @click="checkFact"
        class="w-full mt-4 px-5 py-3 rounded-lg text-white font-bold text-lg shadow transition
  ...
  <!-- Statuszeile -->
        <div class="flex items-center justify-between">
          <div>
            <span
              v-if="result.result === 'Fakt'"
              class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-base font-semibold border
                     bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20"
            >✅ Fakt</span>

            <span
              v-else-if="result.result === 'Fake'"
              class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-base font-semibold border
        ...
        <div v-if="result.claim" class="rounded-xl border border-border bg-muted/50 p-5">
          ...
          <ul class="space-y-1">
            <li v-for="(claim, idx) in result.claim" :key="idx" class="text-lg font-medium text-card-foreground break-words">
              {{ claim }}
            </li>
          </ul>
        </div>
